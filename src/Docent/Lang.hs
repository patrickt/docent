...
eval (Match (Concat a b)) = do
  a' <- eval a >>= require (O._Term % O._EString) NonStringConcat
  b' <- eval b >>= require (O._Term % O._EString) NonStringConcat
  pure (eString (a' <> b'))
eval v@(Match (Lam _ _)) = pure v
eval (Match (App f x)) = do
  (_, b) <- eval f >>= require (O._Term % O._Lam) NonFunctionApplication
  x' <- eval x
  eval (instantiate1 x' b)
eval (Match (Let e b)) = eval (instantiate1 e b)
eval v@(Match (Record _)) = pure v
eval (Match (Project rec_ f)) = do
  rs <- eval rec_ >>= require (O._Term % O._Record) NonRecordProjection
  field <- require _Just (MissingField f) (OMap.lookup f rs)
  eval field
eval v@(Match (Inject {})) = pure v
eval (Match (Case val branches)) = do
  (lab, _ty, payload) <- eval val >>= require (O._Term % O._Inject) NonVariantScrutinee
...
eval (Match (Unfold e)) = do
  (_ty, inner) <- eval e >>= require (O._Term % O._Fold) NonFoldUnfold
  eval inner
eval v@(Match (Pack {})) = pure v
eval (Match (Unpack _name val scope)) = do
  (payload, _wit, _ann) <- eval val >>= require (O._Term % O._Pack) NonPackUnpack
  payload' <- eval payload
  eval (instantiate1 payload' scope)
eval v@(Match (TyLam {})) = pure v
eval (Match (TyApp e _ty)) = do
  (_name, body) <- eval e >>= require (O._Term % O._TyLam) NonTypeAbstraction
  eval body
